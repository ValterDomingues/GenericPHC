**==============================================================================
* Envio email Relatório de Incidências (Rosto) + PDF
*------------------------------------------------------------------------------
* Diagnóstico / correções face ao sintoma:
*   "corre sem erro, mas o PDF não é criado nem anexado"
*
* Causas mais frequentes neste padrão PHC:
*   1) Pasta backup\ não existe → IduToPdf falha em silêncio ao gravar
*   2) Gensqlp devolve .T. mas SQLTMP está vazio → PDF não sai
*   3) IduToPdf não é validado; o email segue na mesma sem anexo
*   4) Caminho Sys(5)+Curdir() mal formado (usar ADDBS)
*==============================================================================

LOCAL lcBackupDir, llPdfOk, lnSqlTmp, mResult
LOCAL cHtmlArtigos, mIDU
...
* Confirmar que o usqlstamp aponta para a definição SQL correta do relatório
v_usqlstamp = 'Adm26071634087,4110000-1'
u_requery([usql])

If Reccount('usql') = 0
	Mensagem('Não foi encontrada a definição USQL (usqlstamp). Verifique v_usqlstamp.','Directa')
	Return
EndIf

Select usql
u_sqlexec([SELECT ]+usqlfields()+[ FROM usql WHERE usqlstamp=']+ALLTRIM(usql.usqlstamp)+['],[tempsql])

If !Used('tempsql') Or Reccount('tempsql') = 0
	Mensagem('Não foi possível carregar tempsql a partir de usql.','Directa')
	Return
EndIf
...
If !Gensqlp([IduUsql])
	Mensagem('Gensqlp(IduUsql) falhou. Não foi gerado o cursor SQLTMP.','Directa')
	Return
EndIf

*--- Validar o cursor que alimenta o PDF ------------------------------------
If !Used('SQLTMP')
	Mensagem('Gensqlp correu, mas o cursor SQLTMP não existe.','Directa')
	Return
EndIf

Select SQLTMP
lnSqlTmp = Reccount('SQLTMP')
If lnSqlTmp = 0
	Mensagem('SQLTMP está vazio — o PDF não terá conteúdo / pode não ser criado.','Directa')
	* Continuar ou Return conforme regra de negócio; por omissão aborta:
	Return
EndIf

*--- Garantir pasta e caminho absoluto do PDF -------------------------------
lcBackupDir = Addbs(Sys(5) + Curdir()) + 'backup'
If !Directory(lcBackupDir)
	Try
		Md (lcBackupDir)
	Catch
		Mensagem('Não foi possível criar a pasta: ' + lcBackupDir,'Directa')
		Return
	EndTry
EndIf

mIDU = 'Relatório de Incidências (Rosto)'
m.FileName = Addbs(lcBackupDir) + 'RelatorioIncidencia.pdf'

* Remover PDF antigo (evita ficheiro bloqueado / versão stale)
If File(m.FileName)
	Try
		Delete File (m.FileName)
	Catch
		* ignore — IduToPdf pode sobrescrever
	EndTry
EndIf

*--- Gerar PDF --------------------------------------------------------------
* Nota: IduToPdf em alguns ambientes PHC não devolve estado fiável;
* por isso validamos sempre com File() a seguir.
...
llPdfOk = File(m.FileName)
If !llPdfOk
	Mensagem('IduToPdf terminou sem criar o ficheiro:' + Chr(13) + m.FileName + Chr(13) + ;
		'SQLTMP tinha ' + Transform(lnSqlTmp) + ' registo(s).','Directa')
	Return
EndIf
...
Text To m.EmailBody TextMerge NoShow
	...
	<span style='color:#212121;mso-fareast-language:PT'><o:p></o:p></span>
	</p>
	<p class=MsoNormal align=center style='text-align:center;background:white'>
...
*--- 3. Enviar email (só se o PDF existir) ---------------------------------
...
If mResult != 0
	DeactForm()
	Mensagem('O relatório da incidência nº ' + Alltrim(Transform(u_reccli.norec)) + ;
		' não foi enviado. Verifique a sua ligação à internet e tente novamente.','Directa')
Else
	Mensagem('Email enviado com sucesso (PDF: ' + m.FileName + ')','Directa')
EndIf
